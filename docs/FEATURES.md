...
| ✅ | Home (`/sessions`): session list grouped by update time, status dot (idle/running/awaiting/archived/error), model + mode row, relative timestamp, live WS connection indicator. Calendar + Settings2 header buttons are gone — navigation is the tab bar; a small projects-management gear remains in the header for rename/delete | `web/src/screens/Home.tsx` |
...
| ✅ | **Chat header (desktop ≥md)**: status dot + title/meta on the left; right rail is ViewMode dropdown, Model pill, Permission-mode pill, context ring, /btw, session settings, terminal. Model and permission-mode pills are real dropdowns bound to `PATCH /api/sessions/:id` — no round-trip through the session settings sheet for the common case | same |
...
| ✅ | **View modes (Normal / Verbose / Summary)** — dropdown picker in the desktop Chat header (via `ViewModePicker`) and under the mobile three-dot menu as a 3-way tile row. **Normal** collapses non-diff tool_use calls to single-line chips (click to expand to full pretty-printed input), truncates tool_result to 1200 chars with a "show N more chars" toggle, hides thinking blocks entirely. **Verbose** expands every piece, never truncates tool_result, and surfaces thinking blocks (requires the Agent SDK to be started with `thinking.display: "summarized"` — see the Chat-loop row). **Summary** keeps only user messages + the final `assistant_text` of each assistant turn and appends an **Outcome** card (driven by `session.status`) and a **Changes** card that aggregates `Edit`/`Write`/`MultiEdit` tool calls into per-file `+`/`−` line totals (PR card from mockup s-07 is still planned — no git integration yet). Session-scoped, no persistence across reloads | `web/src/screens/Chat.tsx`, `web/src/components/ViewModePicker.tsx`, `web/src/state/sessions.ts` |
